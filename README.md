...
This folder is the independent implementation boundary for VideoTokenPress.
It contains the core framework, DriveVA runtime adapter, evaluator, scripts,
tests and package installer. The parent DriveVA model source is not modified by
this framework; the optional `DriveVAAdapter` patches only the active pipeline
instance at runtime.

The repair requirements and acceptance checklist are recorded in
[`VideoTokenPress_框架修复报告_20260828.md`](VideoTokenPress_框架修复报告_20260828.md).
The original design is [`press.md`](../press.md).
...
```bash
/home/cpj/miniconda3/envs/DriveVA/bin/python -m pip install \
...
The default matrix has 18 methods:
...
| `physical` | NoPress; Random/TokenNorm/ActionAttention/ActionAttention-VNorm + KVPrune; SimilarityMerge + KVMerge |
...
Run all 18 methods on the complete corrected official set with six GPUs. The
