...
def aggregate_records(records: Iterable) -> dict:
    ...
    pdm = _finite_mean([row.get("pdm", float("nan")) for row in rows])
    trajectory_l2 = _finite_mean([row.get("trajectory_l2", float("nan")) for row in rows])
    endpoint_l2 = _finite_mean([row.get("endpoint_l2", float("nan")) for row in rows])
    latency = _finite_mean([row.get("latency_ms", float("nan")) for row in rows])
    selector_latency = _finite_mean([row.get("selector_latency_ms", float("nan")) for row in rows])
    model_latency = _finite_mean([row.get("model_latency_ms", float("nan")) for row in rows])
    e2e_latency = _finite_mean([row.get("e2e_latency_ms", row.get("latency_ms", float("nan"))) for row in rows])
    memory = _finite_mean([row.get("peak_memory_mb", float("nan")) for row in rows])
    eligible_ratio = _finite_mean([row.get("eligible_keep_ratio", float("nan")) for row in rows])
    history_ratio = _finite_mean([row.get("history_keep_ratio", float("nan")) for row in rows])
    return {
        "n_scenes": len(rows),
        "valid_scenes": sum(bool(row.get("valid", True)) for row in rows),
        "pdm": pdm,
        "pdm_std": _finite_std([row.get("pdm", float("nan")) for row in rows]),
        "trajectory_l2": trajectory_l2,
        "trajectory_l2_std": _finite_std([row.get("trajectory_l2", float("nan")) for row in rows]),
        "endpoint_l2": endpoint_l2,
        "endpoint_l2_std": _finite_std([row.get("endpoint_l2", float("nan")) for row in rows]),
        "latency_ms_mean": latency,
        "latency_ms_std": _finite_std([row.get("latency_ms", float("nan")) for row in rows]),
        "selector_latency_ms_mean": selector_latency,
        "selector_latency_ms_std": _finite_std([row.get("selector_latency_ms", float("nan")) for row in rows]),
        "model_latency_ms_mean": model_latency,
        "model_latency_ms_std": _finite_std([row.get("model_latency_ms", float("nan")) for row in rows]),
        "e2e_latency_ms_mean": e2e_latency,
        "e2e_latency_ms_std": _finite_std(
            [row.get("e2e_latency_ms", row.get("latency_ms", float("nan"))) for row in rows]
        ),
        "peak_memory_mb_mean": memory,
        "peak_memory_mb_std": _finite_std([row.get("peak_memory_mb", float("nan")) for row in rows]),
        "peak_memory_mb_max": _finite_max([row.get("peak_memory_mb", float("nan")) for row in rows]),
        "eligible_keep_ratio_mean": eligible_ratio,
        "eligible_keep_ratio_std": _finite_std([row.get("eligible_keep_ratio", float("nan")) for row in rows]),
        "history_keep_ratio_mean": history_ratio,
        "history_keep_ratio_std": _finite_std([row.get("history_keep_ratio", float("nan")) for row in rows]),
        "K_mean": _finite_mean([row.get("K", float("nan")) for row in rows]),
        "K_std": _finite_std([row.get("K", float("nan")) for row in rows]),
        "n_candidate_mean": _finite_mean([row.get("n_candidate", float("nan")) for row in rows]),
        "k_length_before_mean": _finite_mean(
            [row.get("metadata", {}).get("k_length_before", float("nan")) for row in rows]
        ),
        "k_length_after_mean": _finite_mean(
            [row.get("metadata", {}).get("k_length_after", float("nan")) for row in rows]
        ),
        "v_length_after_mean": _finite_mean(
            [row.get("metadata", {}).get("v_length_after", float("nan")) for row in rows]
        ),
        "theoretical_attn_ratio_mean": _finite_mean(
            [row.get("metadata", {}).get("theoretical_attn_ratio", float("nan")) for row in rows]
        ),
    }
