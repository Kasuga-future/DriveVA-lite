...
class BasePipeline(torch.nn.Module):
    ...
    def check_resize_height_width(self, height, width, num_frames=None):
        ...
        if num_frames % self.time_division_factor != self.time_division_remainder:
            ...
            print(
                f"num_frames % {self.time_division_factor} != {self.time_division_remainder}. "
                f"We round it up to {num_frames}."
            )
    ...
    def preprocess_video(self, video, torch_dtype=None, device=None, pattern="B C T H W", min_value=-1, max_value=1):
        video = [
            self.preprocess_image(image, torch_dtype=torch_dtype, device=device, min_value=min_value, max_value=max_value)
            for image in video
        ]
    ...
    def download_if_necessary(self, use_usp=False):
        if self.path is None:
            if self.model_id is None:
                raise ValueError(
                    'No valid model files. Please use `ModelConfig(path="xxx")` or '
                    '`ModelConfig(model_id="xxx/yyy", origin_file_pattern="zzz")`.'
                )
...
class PipelineUnitRunner:
    ...
    def __call__(self, unit: PipelineUnit, pipe: BasePipeline, inputs_shared: dict, inputs_posi: dict, inputs_nega: dict) -> Tuple[dict, dict, dict]:
        if unit.take_over:
