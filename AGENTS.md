...
### 2026-09-24（续 4） — A1 训练跑完 + 评测结果：负面但**不能判定 Route A**（配置错误）

- **训练成功**：`STAGE2 END rc=0`，16 个检查点（step-250…3768），启动验证
  `trainable route_a == trainable dit == 177.48M`。
- **评测（1024 场景 / 4 卡 / 配对）**：no_press **0.911078**；Route A 最终检查点 τ=0.5
  **0.540180**，实际保留 788/1560（50%）。**配对 ΔPDM = −0.3709。**
- **但这次负面结果不能判定 Route A**，因为 A1 配置有三处我的错误：
  1. **训练损失没有稀疏项**——我把 Route A 挂到标准 FM 循环上，没有 `λ_sparse`。
     后果：step-1250 保留率 99.7%（几乎不压缩），到 step-3768 自己漂到 50%。
     保留率**失控**，不是被设计控制。
  2. **无意中训练了 trajectory_encoder / trajectory_head**（只冻结了 `dit`），
     而首次评测**只加载 Route A 权重** → 训练/评测不匹配。已修（评测现会一起加载）。
  3. **没有把 `CompressionStatsRecorder` 接进训练循环**——计划 §34–35 要求的保留率
     记录我漏了，所以漂移一直没被发现。
- **观察（尚不可归因）**：step-1250 在 99.7% 保留时 PDM 仅 0.744；但该读数同样被第 2 条
  污染，不能据此说"插入门控+还原器本身掉 0.17"。
- **P0 修正清单**：① 损失加 `λ_sparse` + KD（用 `compute_route_a_loss`）；
  ② 训练循环接入保留率统计与 `is_truly_dynamic`；③ 冻结除 Route A 外一切，
  或明确纳入加载集合；④ A1 末尾切 `physical_shortening=True` 收敛机制差；
  ⑤ 无夹 τ 标定（计划 §25）；⑥ 与同保留率**冻结剪枝**对比，隔离"排序能力 vs 机制代价"。
...
### 2026-09-25（续） — A1 重跑判决：Route A 在 25% 预算下不可用，且学到的排序不优于随机

**结论报告**：`ROUTE_A_A1_VERDICT_20260925.md`（完整表格与复现命令）。

**训练**（`outputs/route_a_train2_20260924/`）：
- 阶段 1 dense-gate：2 卡 × 6 epoch = 11,304 步，1.03 it/s，rc=0，12 个检查点。
- 阶段 2 physical tail：4 卡 × 2 epoch，从阶段 1 热启动，rc=0。
- **保留率终于被控住了**：全程 0.239–0.251（目标 0.25），τ 在 ~2000 步内收敛。
  对比上一轮 A1 的 99.7%→50% 失控漂移，这是本轮 P0 修正有效的直接证据。

**评测（1024 场景配对，4 rank × 256，no_press = 0.911078）**：

| arm | PDM | ΔPDM vs NoPress | 95% CI | 保留 |
|---|---:|---:|---|---:|
| no_press | 0.9111 | — | — | 1560 (100%) |
| Route A dense-gate 检查点（按物理模式评） | 0.7077 | −0.2034 | [−0.2271,−0.1802] | 380 (24.4%) |
| **Route A physical tail 检查点** | **0.7422** | **−0.1688** | [−0.1921,−0.1464] | 353 (22.6%) |
| matched random ×4 | 0.7501–0.7624（mean 0.7567） | −0.1487…−0.1610 | — | ~351 |

**判决性比较**：
- `route_a_physical − mean(random)` = **−0.0145，CI [−0.0317,+0.0021]** → **与随机不可区分**。
- `route_a_densegate − mean(random)` = **−0.0490，CI [−0.0706,−0.0279]** → **显著差于随机**。

**三条可站住的结论**：

1. **Route A 在 ~23% 保留率下掉 0.169 PDM**，离近无损门槛（CI 下界 > −0.002）差两个数量级。
2. **训练出来的排序不优于随机**。随机对照不是普通 top-k：它**置换同一检查点自己的分数**
   （domain 内），因此阈值、保留数、分数直方图完全一致，只有"保哪些"变了。
   即"保留分数最高的 K 个"并不优于"均匀随机保留 K 个"。
3. **dense-gate 松弛不可迁移，且比随机还差**。physical tail 把机制损失救回 **+0.0346**
   （0.7077→0.7422），这部分 P0 有效；但救不回排序能力。
4. 与历史冻结剪枝对照：同样量级预算下 Route A 更差（冻结 random 在 keep0.346 只掉 0.095，
   Route A 在 keep0.226 掉 0.155）。物理删除 78% 的 hidden 再跑 12 层，比 press 的
   "置零但保留序列长度"苛刻得多。

**次要机制发现（值得单独记）**：physical 训练会**破坏门控的可控性**——
dense-gate 阶段 score_std 稳定在 0.192[0.143,0.218]，physical 阶段掉到 0.114[0.043,0.213]；
τ_future 变 0.016 就能让 future 保留率从 0.04 跳到 0.42。分数质量集中在阈值附近时，
门保住的其实是那片平台上的近似任意子集。这是继"每场景常数偏移"之后的**第二个独立刀刃**。

**没测的**：A2/A3（解冻 DiT / LoRA）。A1 冻结骨干，只测了"选择"这一半，而失败的正是这一半。
计划书的完整前提是"重训让模型改变 token 的含义"，那是更大的命题；选择侧的证据给了它的上限。

**建议**：Route A 停在 A1。若要继续，优先级是
①直接用 pairwise/contrastive 损失训排序（不再只靠退化掩码的 STE 梯度）；
②用**每场景分位数 top-K** 固定预算（同时消掉两个刀刃）；
③**先**证明选择在目标预算下有信号，再谈 A2/A3。

**本轮修掉的 bug（会导致评测静默失效）**：`AUTO_EVAL=0` 走基类 `ModelLogger`，
而收尾钩子挂在 auto-eval 子类上 → `route_a_stats_calibration.json` 从未写出 →
